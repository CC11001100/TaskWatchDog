...
class MessageSender:
    ...
    def init_itchat(self):
        try:
            itchat.auto_login(hotReload=True, enableCmdQR=configuration['use_shell_qrcode'], exitCallback=self.wei_xin_log_out)    # 没有GUI界面的情况下应该将这个置为True
    ...
    def wei_xin_log_out(self):
        """
        微信退出登录回调事件
        :return:
        """
        logging.error('Wei Xin logout.')
        sys.exit(-1)
